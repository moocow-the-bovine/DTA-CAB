...
feature-set.  All token elements remain encoded as C<E<lt>wE<gt>> elements (and not e.g. C<E<lt>pcE<gt>> elements),
and only the C<att.linguistic> features C<@lemma>, C<@pos>, and C<@join> are inserted, so that e.g.
...
Note that the C<@join> attribute will only be correctly generated if the relevant C<w> elements are truly immediately adjacent
in the input file: any intervening newline or whitespace will prohibit insertion of a C<@join> attribute.
