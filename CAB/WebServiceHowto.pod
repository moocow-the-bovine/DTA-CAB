...
C<HIDDEN> is either "yes" or "no", and indicates whether the attribute's value is suppressed in the
...
If present, indicates that the surface form occurs in at least one
L<Integrated Authority File|https://www.dnb.de/EN/Standardisierung/GND/gnd_node.html>
record for an author whose work is included in the I<Deutsches Textarchiv> core corpus.
...
[L<example|http://./elephant.raw> (input only)]

Input-only format for untokenized raw UTF-8 text, implicitly used for query strings
...
model characteristic for underlying the C<DTA::CAB> server (typically L<C<de-dta-tiger>|http://kaskade.dwds.de/waste/downloads.perl>).
...
The C<DTA::CAB> source code distribution on L<CPAN|https://metacpan.org/release/DTA-CAB>.
Note however that the source code distribution alone is B<not> sufficient to set up and run
a complete C<DTA::CAB> analysis pipeline on your local machine.  In order to do that, you would
also need various assorted language models and additional resources which are not themselves part
of C<DTA::CAB>, and therefore not part of the source code distribution.  See the
L<Jurish (2012)|http://./#jurish2012> and the source code L<documentation|https://metacpan.org/release/DTA-CAB> for
more details.
