...
Additionally, POST requests with a 'Content-Type' header
other than 'application/x-www-form-urlencoded' or 'multipart/form-data'
are assumed to contain all `option-like' query parameters in the URL, and
the POST content is interpreted as the value of the 'qd' (query document) parameter.
...
in the format and encoding specified by the
L<'fmt'|/"fmt =E<gt> $fmt">
and
L<'enc'|/"enc =E<gt> $enc">
parameters.
...
Specifies the format of the document passed in via the "qd" parameter or
POST request content (see below), and requests that the analyzed document
be returned in the same format.  $fmt should be a
...
Encoding with which to interpret query data and in which the
formatted analysis output document is to be returned.
Must be compatible with the selected $fmt.
