...
## Description: Datum parser: raw untokenized text (using moot/waste) : TODO
...
BEGIN {
  DTA::CAB::Format->registerFormat(name=>__PACKAGE__, short=>'raw-waste', filenameRegex=>qr/\.(?i:raw-waste|txt-waste|raw)$/);
}
...
##     ttfmt => $ttfmt,                ##-- underlying tt format (hack)
##
##     ##-- Common
...
sub new {
  ...
  my $fmt = bless({
		   ...
		   wannotator => undef,
		   wwriter  => undef,
		   ttfmt    => DTA::CAB::Format::TT->new(utf8=>1),
...
sub noSaveKeys {
  return (shift->SUPER::noSaveKeys(), qw(doc wscanner wlexer wtagger wdecoder wannotator wwriter ttfmt));
}
...
sub close {
  delete($_[0]{doc});
  $_[0]{ttfmt}->close(@_[1..$#_]) if ($_[0]{ttfmt});
...
sub fromFh {
  ...
  ##-- hack: tweak string and pass it to tt reader
  $ttstr =~ s{\t\[([^\]\t]+)\]}{\t[toka] $1}mg;
  return undef if (!$fmt->{ttfmt}->parseTTString(\$ttstr));
  $fmt->{doc} = $fmt->{ttfmt}{doc};
  return $fmt;
}
