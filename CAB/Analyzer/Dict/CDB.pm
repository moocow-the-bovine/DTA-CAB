## -*- Mode: CPerl -*-
##
## File: DTA::CAB::Analyzer::Dict::CDB.pm
## Author: Bryan Jurish <moocow@cpan.org>
## Description: generic analysis dictionary API using Lingua::TT::CDBFile

package DTA::CAB::Analyzer::Dict::CDB;
...
##    (
##     ##-- Filename Options
##     dictFile => $filename,    ##-- CDB filename (default=undef)
##
##     ##-- Analysis Output
...
##     dba => \%dbopts,          ##-- options for underlying db: default: {mode=>'r',utf8=>1},
##    )
sub new {
  my $that = shift;
  my $dic = $that->SUPER::new(
			      ...
			      ##-- analysis output
			      label => 'dict',
			      analyzeCode => ('utf8::decode '.$DTA::CAB::Analyzer::Dict::CODE_DEFAULT),
...
sub ensureLoaded {
  ...
  if ( defined($dic->{dictFile}) && !$dic->dictOk ) {
    $dic->info("opening CDB file '$dic->{dictFile}'");
    $dic->{dbf} = Lingua::TT::CDBFile->new(%{$dic->{dba}||{}});
    $rc &&= $dic->{dbf}->open($dic->{dictFile});
  }
  $dic->logwarn("error opening CDB file '$dic->{dictFile}': $!") if (!$rc);
  return $rc;
}
...
This module uses Lingua::TT::CDBFile to implement a static
finite dictionary stored in a CDB file.
...
##----------------------------------------------------------------
## DESCRIPTION: DTA::CAB::Analyzer::Dict::CDB: Constructors etc.
=pod
...
##----------------------------------------------------------------
## DESCRIPTION: DTA::CAB::Analyzer::Dict::CDB: Methods: Embedded API
=pod
...
##----------------------------------------------------------------
## DESCRIPTION: DTA::CAB::Analyzer::Dict::CDB: Methods: I/O: Input: all
=pod
...
Ensures analyzer data is loaded from default files.
Override instantiates $dic-E<gt>{dbf} as a new Lingua::TT::CDBFile object.
...
##----------------------------------------------------------------
## DESCRIPTION: DTA::CAB::Analyzer::Dict::CDB: Methods: Persistence: Perl
=pod
